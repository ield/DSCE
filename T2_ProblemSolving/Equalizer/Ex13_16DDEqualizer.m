clear;
% Trying to minimize the distance between the quantization and the expected
% result
tests  =30;
error = zeros(tests);
...
for n = 1:tests
    ...
    f(ceil(n/2)) = 1;
    e_fin = zeros(1, (m-(n+1)));
    for delta = 1:tests               %delya <= n+2 for lin 20
        ...
        for ii=n+1:m                  % iterate
          ...
          e_fin=sign(f'*rr)-f'*rr;       % calculate error
        end

        error(n, delta) = min(sum(abs(e_fin)), 1500);
        minError = min(error(n, delta), minError);
        
    end
end
[n, delta] = find(error==minError, 1);
...
fprintf('Since there is quantization almost any delay as long as it is short is ok');
